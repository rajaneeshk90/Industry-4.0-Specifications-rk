...
- REQUIRED. The BAP MUST implement the `on_search` endpoint to consume the `Catalog` objects containing 3d-printing Products sent by BPPs.
...
- REQUIRED. If the 3d-printing provider has successfully received the information submitted by the 3d-printing consumer, the BPP must return an acknowledgement with `ack.status` set to `ACK` in response to the `select` request
- REQUIRED. If the 3d-printing provider has returned a successful acknowledgement to a `select` request, it MUST send the offer encapsulated in an `Order` object
